...
:: --- CSV Processing Loop -------------------------------------
:: eol=# : lines starting with # are auto-skipped
:: for/f strips trailing CR so Windows CRLF CSV files work safely
:: Nested if instead of goto - goto inside for/f exits the whole loop
call :log "Reading: !INPUT_CSV!"
echo.

for /f "usebackq eol=# tokens=1,2 delims=," %%A in ("!INPUT_CSV!") do (
    set "_APP=%%~A"
    set "_IMG=%%~B"

    :: Trim leading spaces
    for /f "tokens=* delims= " %%T in ("!_APP!") do set "_APP=%%T"
    for /f "tokens=* delims= " %%T in ("!_IMG!") do set "_IMG=%%T"

    :: Skip blank or header rows
    if not "!_APP!"=="" if not "!_IMG!"=="" (
        echo !_APP! | findstr /i /c:"app_name" /c:"application_name" >nul 2>&1
        if errorlevel 1 (
            set /a G_TOTAL+=1
            call :log "----------------------------------------------------------"
...
:process_app
    ...
    if "!PA_FOUND!"=="0" (
        ...
        docker rm -f "!PA_CID!" >nul 2>&1
        if "!KEEP_ARTIFACTS!"=="0" (
            docker rmi -f "!PA_IMG!" >nul 2>&1
            if exist "!PA_WDIR!" rd /s /q "!PA_WDIR!" >nul 2>&1
        )
        set /a G_FAIL+=1
        goto :eof
    )
