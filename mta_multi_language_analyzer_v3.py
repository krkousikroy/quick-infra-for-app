...
# Import resource module only on Unix systems (not available on Windows)
try:
    import resource
    HAS_RESOURCE = True
except ImportError:
        ...
        def scan_app_type(app_type: str, config: Dict[str, Any]) -> Tuple[str, int, List[str]]:
            ...
            with scan_semaphore:
                try:
                    ...
                    def set_limits():
                        # Limit memory to 512MB (only on Unix systems)
                        if HAS_RESOURCE:
                            try:
                                resource.setrlimit(resource.RLIMIT_AS, (512 * 1024 * 1024, 512 * 1024 * 1024))  # type: ignore
                            except:
