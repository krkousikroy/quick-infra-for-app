...
def is_library_artifact(path):
    ...
    lib_dirs = [
        '/modules/', '/system/', '/var/lib/', '/usr/lib/', '/usr/share/', '/opt/jboss/wildfly/modules/',
        '/opt/jboss/eap/modules/', '/opt/jboss/wildfly/standalone/tmp/', '/opt/jboss/wildfly/standalone/data/'
    ]
    if any(segment in lower_path for segment in lib_dirs) and '/deployments/' not in lower_path:
        return True

    blacklist = [
        'junit', 'test', 'mock', 'log4j', 'slf4j', 'hibernate', 'tomcat', 'wildfly', 'jboss',
        'xerces', 'api', 'client', 'commons', 'spring-core', 'spring-context', 'spring-beans',
        'spring-jdbc', 'spring-web', 'spring-boot-starter', 'servlet', 'jsp', 'validation',
        'jackson', 'json', 'xml', 'logback', 'mariadb', 'mysql', 'postgresql', 'jdbc',
        'el-api', 'javax', 'jakarta', 'jaxb', 'activation', 'apache', 'httpclient', 'httpcore'
    ]
...
def choose_application_artifacts(paths, app_name):
    scored = [(score_application_artifact(p, app_name), p) for p in paths]
