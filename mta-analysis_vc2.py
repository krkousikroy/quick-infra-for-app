...
def check_mta_api(url: str, token: str, ca_bundle: str = None):
    logging.info("Testing MTA API connection...")
    headers = {"Authorization": f"Bearer {token}"} if token else {}
    try:
        verify = ca_bundle if ca_bundle else False
        response = requests.get(f"{url}/hub/applications", headers=headers, verify=verify, timeout=10)
...
def process_app_row(app_name: str, current_image: str, namespace: str, url: str, token: str, targets_csv: str, cleanup: bool, skip_oc: bool, ca_bundle: str = None) -> bool:
...
def main():
    ...
    parser.add_argument("--ca-bundle", help="Path to CA bundle file for SSL verification")
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        ...
        api_future = executor.submit(check_mta_api, url_clean, args.token, args.ca_bundle)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_app = {
            executor.submit(process_app_row, app_name, img_url, ns, url_clean, args.token, args.targets, not args.keep, args.skip_oc, args.ca_bundle): app_name 
