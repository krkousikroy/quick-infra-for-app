...
def process_app_row(app_name, current_image, namespace, url, token, targets_csv, cleanup, skip_oc):
    ...
    ignore_substrings = [
        ...
        "/lib/", "/libs/", "WEB-INF/lib/", "BOOT-INF/lib/"
    ]
    ...
    try:
        ...
        for artifact in main_artifacts:
            ...
            with open(artifact_path, "rb") as f:
                ...
                # Use application/octet-stream to safely upload jar, war, ear without strict MIME validation issues
                upload_resp = requests.post(
                    f"{url}/hub/taskgroups/{tg_id}/bucket/",
                    headers=upload_headers,
                    files={"file": (f"binary/{artifact}", f, "application/octet-stream")},
                    verify=False
                )
                if upload_resp.status_code in [200, 201, 204]:
                    uploaded_files_count += 1
                else:
                    logging.error(f"TaskGroup file upload failed for {artifact} with HTTP code: {upload_resp.status_code}")
        ...
        # Safely reuse the local assessment_data payload instead of the server's response
        # to avoid trying to overwrite read-only audit fields (createUser, dates, etc.)
        assessment_data["id"] = tg_id
        assessment_data["data"]["mode"]["artifact"] = f"/hub/taskgroups/{tg_id}/bucket/binary/{main_artifacts[0]}"
        
        update_resp = requests.put(f"{url}/hub/taskgroups/{tg_id}", headers=headers, json=assessment_data, verify=False)
        ...
        logging.error(f"Failed to start assessment or form taskgroup logic: {e}")
        step_cleanup(app_name, current_image, container_id, extract_dir, cleanup)
        return False
