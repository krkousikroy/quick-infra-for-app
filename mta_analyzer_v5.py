...
class MTAAnalyzer:
    ...
    def __init__(self, mta_url: str, output_dir: str = 'artifacts', cleanup: bool = True):
        """
    ...
    def cleanup_all_artifacts(self) -> None:
        ...
        if not os.path.exists(self.output_dir):
            logger.debug(f"Artifacts directory does not exist: {self.output_dir}")
            return
        
        try:
            # Get list of app directories
            app_dirs = [d for d in os.listdir(self.output_dir)
                       if os.path.isdir(os.path.join(self.output_dir, d))]
            
            if not app_dirs:
                logger.debug("No artifacts to clean up")
                return
            
            logger.info(f"\nCleaning up artifacts for {len(app_dirs)} applications...")
            
            for app_name in app_dirs:
                self.cleanup_artifacts(app_name)
            
            # Remove the artifacts directory if empty
            if not os.listdir(self.output_dir):
                os.rmdir(self.output_dir)
                logger.info(f"Removed empty artifacts directory: {self.output_dir}")
                
        except Exception as e:
            logger.warning(f"Error during artifacts cleanup: {e}")
...
def main():
    ...
    analyzer = None
    try:
        analyzer = MTAAnalyzer(args.mta_url, output_dir=args.output_dir, cleanup=cleanup_enabled)
