...
def create_requests_session(verify=False):
...
def check_mta_api(url, token, session):
    ...
    try:
        response = session.get(f"{url}/hub/applications", headers=headers, timeout=10)
...
def process_app_row(app_name, current_image, namespace, url, token, targets_csv, cleanup, skip_oc=False, session=None):
    ...
    try:
        app_resp = session.post(f"{url}/hub/applications", headers=headers, json=app_payload, timeout=30)
    ...
    try:
        tg_resp = session.post(f"{url}/hub/taskgroups", headers=headers, json=assessment_data, timeout=30)
        ...
        submit_resp = session.post(f"{url}/hub/taskgroups/{tg_id}/submit", headers=submit_headers, timeout=30)
