...
def check_mta_api(url: str, token: str):
    ...
    try:
        response = requests.get(f"{url}/hub/applications", headers=headers, verify=False, timeout=15)
        response.raise_for_status()
        logging.info("MTA API connection successful.")
    except requests.exceptions.RequestException as e:
        status_msg = f"HTTP {e.response.status_code}" if e.response is not None else str(e)
        logging.error(f"MTA API connection error: {status_msg}")
        sys.exit(1)
